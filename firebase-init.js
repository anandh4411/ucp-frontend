  ...
  try {
    ...
    for (const userData of users) {
      try {
        ...
        await db.collection('user_preferences').doc(userRecord.uid).set({
    ...
    const announcements = [
      {
        ...
        content: 'All personnel are required to attend the Republic Day Parade on 26th January 2025 at 0800 hrs at Parade Ground. Full ceremonial dress with medals.',
        priority: 'urgent',
        category: 'event',
        isPinned: true,
        authorId: createdUsers.find(u => u.profile.role === 'adjt')?.uid || '',
        readBy: [],
        createdAt: admin.firestore.FieldValue.serverTimestamp(),
        updatedAt: admin.firestore.FieldValue.serverTimestamp()
      },
      {
        ...
        content: 'Weapon training will be conducted from 15-20 January 2025. All personnel must report to training area by 0600 hrs. Bring personal weapons and 30 rounds ammunition.',
        priority: 'high',
        category: 'training',
        isPinned: false,
        authorId: createdUsers.find(u => u.profile.role === 'adjt')?.uid || '',
      ...
      },
      {
        ...
        content: 'The leave roster for Q1 2025 has been updated. All pending leave applications have been processed. Check notice board for details.',
        priority: 'normal',
        category: 'administrative',
        isPinned: false,
        authorId: createdUsers.find(u => u.profile.role === 'it_jco')?.uid || '',
        readBy: [],
        createdAt: admin.firestore.FieldValue.serverTimestamp(),
        updatedAt: admin.firestore.FieldValue.serverTimestamp()
      }
    ...
    const events = [
      {
        ...
        description: 'Weekly operational briefing by Commanding Officer. All JCOs and above must attend.',
        startTime: admin.firestore.Timestamp.fromDate(new Date('2025-01-20T09:00:00')),
        endTime: admin.firestore.Timestamp.fromDate(new Date('2025-01-20T10:00:00')),
        location: 'Conference Room, HQ Building',
        category: 'meeting',
        organizerId: createdUsers.find(u => u.profile.role === 'adjt')?.uid || '',
        attendeeIds: [],
        isMandatory: true,
        isAllDay: false,
        reminderBefore: 60,
      ...
      },
      {
        ...
        startTime: admin.firestore.Timestamp.fromDate(new Date('2025-01-15T06:00:00')),
        endTime: admin.firestore.Timestamp.fromDate(new Date('2025-01-15T07:30:00')),
      ...
      },
      {
        ...
        startTime: admin.firestore.Timestamp.fromDate(new Date('2025-01-26T08:00:00')),
        endTime: admin.firestore.Timestamp.fromDate(new Date('2025-01-26T12:00:00')),
    ...
    const resources = [
      {
        ...
        description: 'Complete training manual for all personnel - updated for 2025',
        fileName: 'training-manual-2025.pdf',
        fileUrl: '', // Will be uploaded separately
        fileSize: 1024 * 1024 * 2, // 2MB placeholder
        fileType: 'pdf',
        category: 'training',
        uploadedById: createdUsers.find(u => u.profile.role === 'adjt')?.uid || '',
      ...
      },
      {
        ...
        fileType: 'document',
        category: 'operations',
        uploadedById: createdUsers.find(u => u.profile.role === 'it_jco')?.uid || '',
    ...
    if (adjt && user) {
      const conversationRef = await db.collection('conversations').add({
        ...
        lastMessage: {
          content: 'I would like to apply for leave next month. What is the process?',
